...
private slots:
    ...
    void testSwitchOffVFS()
    {
        ...
        fakeFolder.localModifier().insert(fileName(QStringLiteral("A/a3")), 100_b); // TODO: should this really have the suffix-vfs file suffix??
        QVERIFY(fakeFolder.applyLocalModificationsWithoutSync());
        ...
        if (vfsMode == Vfs::WithSuffix) {
            QVERIFY(!fakeFolder.currentRemoteState().find(fileName(QStringLiteral("A/a3")))); // regular upload
        } else {
            QVERIFY(!fakeFolder.currentRemoteState().find(QStringLiteral("A/a3"))->isDehydratedPlaceholder);
        }
